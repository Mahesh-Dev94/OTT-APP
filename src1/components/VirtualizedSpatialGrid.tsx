...
interface RowStyle extends ViewStyle {
  gap?: number;
}
...
export const VirtualizedSpatialGrid = ({data, containerStyle }: { containerStyle?: ViewStyle,data:any }) => {
  const renderItem = useCallback(({ item }:any) => <ProgramNode programInfo={item} />, []);
...
const styles = StyleSheet.create({
  ...
  rowStyle: { gap: scaledPixels(30) } as RowStyle,
  topArrowContainer: {
    width: '100%',
    height: 100,
    position: 'absolute',
